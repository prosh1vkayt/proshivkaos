...
make mido                 # образ для Redmi Note 4/4X (см. docs/PORT_MIDO.md)
make help                 # все цели
```
...
| [docs/PORT_MIDO.md](docs/PORT_MIDO.md)       | порт на Redmi Note 4/4X: что проверено, что нет |
...
драйвера тачскрина по I2C для реальных телефонов.

Адреса периферии на ARM64 больше **не** зашиты в код: система читает device
tree, который передаёт загрузчик. Разбор проверен и на дереве от QEMU, и на
вытащенном из стокового образа Redmi Note 4. Что осталось сделать для
конкретного телефона — в [docs/PORT_MIDO.md](docs/PORT_MIDO.md).
