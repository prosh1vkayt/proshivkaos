...
typedef struct {
    ...
    const char *glyph;   /* 1-3 символа внутри иконки — если нет векторной */
    int         icon;    /* ICON_* из gui/icon.h; 0 и glyph — по-старому  */
    ...
    /* Необязательно (может быть 0). Экран под приложением перерисовал
     * кто-то другой — рабочий стол, отметка касания, — и следующий render
     * должен быть полным. Приложения, которые всегда рисуют всё целиком,
     * его не заводят; терминал рисует только изменившиеся клетки, и ему
     * без этого сигнала на экране остались бы чужие пиксели. */
