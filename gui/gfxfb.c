...
void gfxfb_present(void) {
    ...
    if (g_format == GFXFB_FMT_PAL8) {
        ...
        /* Три байта на пиксель, без выравнивающего байта — именно так
           ...
           зелёный, потом красный. Название "r8g8b8" из описания
           simple-framebuffer перечисляет разряды значения от старшего к
           младшему (в терминах DRM это RGB888), а при обратном порядке
           байтов старший разряд ложится в памяти последним. Проверено на
           живом устройстве: полосы, закрашенные как R,G,B, выходили на
           экране синей и красной наоборот. */
        for (int y = 0; y < g_height; y++) {
            volatile uint8_t *dst = g_fb + (long)y * g_pitch;
            const uint8_t *src = &g_backbuffer[(long)y * g_width];
