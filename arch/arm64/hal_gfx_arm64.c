...
void hal_gfx_init(void) {
    ...
    if (pi->fb_valid) {
        ...
        /* Кадр загрузчика лежит в памяти, общей с контроллером дисплея, и
           отображён некэшируемым (см. mmu.c). Выталкивать после отрисовки
           нечего — а обход всего кадра строками кэша стоил столько же,
           сколько сама отрисовка. */
        gfxfb_set_uncached(1);
