...
/* НОМЕР ФУНКЦИИ ВЫВОДА. Двойка, а не тройка.
 *
 * В таблице выводов msm8953 у gpio10 и gpio11 перечислены
 * blsp_spi3, blsp_i2c3, gcc_gp2_clk_b — и заманчиво посчитать I2C
 * третьим. Но нулевой номер занимает сам GPIO, поэтому blsp_spi3 это
 * единица, а blsp_i2c3 — двойка.
 *
 * С тройкой выводы уходили на выход тактового сигнала, и контроллер
 * шины оказывался ни к чему не подключён: он честно объявлял ошибку
 * шины ещё до первого байта, а разобрать это было невозможно — на той
 * же шине ещё и не было запитанного устройства. */
#define BOARD_I2C_TS_PIN_FUNC  2
