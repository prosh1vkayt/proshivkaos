/* arch/arm64/uart_pl011.c — драйвер ARM PL011, прямой аналог arch/x86/vga.c.
 *
 ...
 *
 * Наружу этот файл больше не торчит: uart_* теперь диспетчер в
 * arch/arm64/uart.c, который на старте выясняет по device tree, какой
 * контроллер стоит на плате, и вызывает pl011_* либо msm_uart_* из
...
/* Адрес приходит от диспетчера: он берёт его из device tree, а к
 * захардкоженной константе откатывается, только если дерева нет. */
