...
void virtio_input_init(void) {
    ...
    /*
     * СНАЧАЛА УБЕДИМСЯ, ЧТО ТУТ ВООБЩЕ ЕСТЬ ЧТО ОПРАШИВАТЬ.
     *
     * Ниже мы читаем регистры по адресу 0x0A000000 — это раскладка машины
     * QEMU virt, и больше нигде она ничего не значит. На телефоне по
     * этому адресу лежит либо чужой блок, либо ничего, а чтение
     * несуществующего регистра на Snapdragon не возвращает мусор, а
     * вешает шину: процессор ждёт ответа, которого не будет.
     *
     * Поэтому спрашиваем дерево устройств. Есть узел virtio — опрашиваем;
     * дерево разобрано и узла нет — уходим молча, это не ошибка, а другая
     * плата. Дерева нет вовсе — пробуем по старой памяти, потому что
     * тогда мы почти наверняка под эмулятором, который дерева не дал.
     */
    if (fdt_valid()) {
        fdt_node_t node;
        if (!fdt_find_compatible("virtio,mmio", &node)) {
            uart_write("virtio-input: v dereve ustroystv net, propuskaem opros\n");
            return;
        }
    }
