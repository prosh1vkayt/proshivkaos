...
/* Последовательность изготовителя для QUSB2 на этом процессоре. Числа
 * взяты из дерева устройств телефона (qcom,qusb-phy-init-seq) и совпадают
 * с тем, что пишет загрузчик. Смысл у них электрический — сопротивление
 * линий, ток передатчика, настройки ФАПЧ, — и подбирать их самим нельзя. */
static int qusb2_phy_init(void) {
    const uint64_t phy = BOARD_QUSB2_PHY_BASE;

    /* Блок приёмопередатчика — в сброс и обратно. */
    gcc_usb_block_reset(1);

    /* Шина к регистрам приёмопередатчика: одно состояние ожидания. */
    mmio_write32(BOARD_USB_AHB2PHY_BASE + AHB2PHY_TOP_CFG, 0x11);
    dsb_sy();

    /* Держим приёмопередатчик выключенным, пока настраиваем. */
    mmio_write32(phy + QUSB2_PORT_POWERDOWN, 0x23);
    dsb_sy();

    mmio_write32(phy + QUSB2_PORT_TUNE1, 0xF8);
    mmio_write32(phy + QUSB2_PORT_TUNE2, 0xB3);
    mmio_write32(phy + QUSB2_PORT_TUNE3, 0x83);
    mmio_write32(phy + QUSB2_PORT_TUNE4, 0xC0);
    mmio_write32(phy + QUSB2_PLL_TUNE, 0x30);
    mmio_write32(phy + QUSB2_PLL_USER_CTL1, 0x79);
    mmio_write32(phy + QUSB2_PLL_USER_CTL2, 0x21);
    mmio_write32(phy + QUSB2_PORT_TEST2, 0x14);
    mmio_write32(phy + QUSB2_PLL_AUTOPGM_CTL1, 0x9F);
    mmio_write32(phy + QUSB2_PLL_PWR_CTL, 0x00);
    dsb_sy();
    ...
    /* Опорный такт одиночный, а не парный — так разведено на этой плате. */
    mmio_write32(phy + QUSB2_PLL_TEST, 0x80);
    dsb_sy();
