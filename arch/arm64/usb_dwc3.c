...
static int qusb2_phy_init(void) {
    ...
    /* 0. ПИТАНИЕ. Приёмопередатчик питается тремя источниками внутри
     *    микросхемы питания, и загрузчик, уходя, гасит то, чем сам не
     *    пользуется. Пока они не поданы, умножитель частоты не
     *    захватится никогда, сколько ни настраивай остальное. */
    pmic_ldo_dump(3);  pmic_ldo_enable(3);    /* цифровая часть, 0.925 В */
    pmic_ldo_dump(7);  pmic_ldo_enable(7);    /* аналоговая часть, 1.8 В */
    pmic_ldo_dump(13); pmic_ldo_enable(13);   /* линия, 3.075 В          */
