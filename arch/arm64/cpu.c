...
void hal_arch_init(void *boot_info) {
     ...
     *
     * СТРОГО ПОСЛЕ ПРОВОДА, и это не мелочь. Сперва он поднимался
     * раньше — и когда рукопожатие с сопроцессором подвесило аппарат,
     * узнать об этом было нечем: провод к тому времени ещё не работал,
     * журнал никуда не шёл, а телефон просто замер. Теперь любой отказ
     * здесь виден на компьютере построчно. */
    smd_rpm_init();
#endif
