...
void hal_arch_init(void *boot_info) {
    ...
    /* ФАКТИЧЕСКИЙ АДРЕС ЗАГРУЗКИ — полосами, потому что прочитать его
     * больше неоткуда: порт молчит, журнал без root не достать.
     *
     * Загрузчик кладёт образ туда, куда считает нужным, и от этого адреса
     * зависит разметка памяти: попадаем мы в область, размеченную как
     * обычная память, или в ту, что отведена регистрам устройств. Ошибка
     * здесь убивает систему ровно на включении MMU.
     *
     * Показываем старший шестнадцатеричный разряд адреса числом полос:
     * одна полоса — образ по 0x1xxxxxxx, восемь — по 0x8xxxxxxx. Считать
     * их на фотографии проще, чем разбирать цвета. */
    {
        extern char _start[];
        unsigned long load = (unsigned long)(uintptr_t)_start;
        int digit = (int)((load >> 28) & 0xF);
        if (digit > 12) digit = 12;            /* ниже полосы заняты */
        for (int i = 0; i < digit; i++)
            early_fb_band(12 + i, 255, 220, 0);
    }
    ...
    /* Щель между включением MMU и этой точкой оказалась смертельной, а
       действий в ней всего два. Размечаем каждое отдельно: полоса 25 —
       вернулись из mmu_init, полоса 24 — пережили первую запись в журнал
       при включённом кэше (до сих пор она была холостой: сброс кэша
       пропускался, пока MMU выключен, и только теперь заработал). */
    early_fb_band(25, 255, 255, 255);   /* белая: вернулись из mmu_init */

    ramoops_write("[4] sistemnyy schetchik\n");
    early_fb_band(24, 200, 0, 255);     /* фиолетовая: журнал пережит */
...
void arm64_exception_handler(uint64_t index, uint64_t esr, uint64_t far, uint64_t elr) {
    ...
    switch ((esr >> 26) & 0x3F) {
        case 0x20: case 0x21:           /* не смогли прочитать команду */
            early_fb_band(13, 255, 255, 255); break;
        case 0x24: case 0x25:           /* не смогли обратиться к данным */
            early_fb_band(14, 255, 255, 255); break;
        case 0x2F:                      /* внешний сбой шины */
            early_fb_band(15, 255, 255, 255); break;
        default:
            early_fb_band(16, 255, 255, 255); break;
    }
