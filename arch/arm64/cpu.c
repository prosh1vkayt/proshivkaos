...
void hal_arch_init(void *boot_info) {
    ...
    dsb_sy();
#elif defined(BOARD_WDOG_BASE)
    /* Сторожевой таймер заводится с длинным сроком вместо того, чтобы
       глушиться. Зависшая система иначе требует кнопки питания и живого
       человека — а круг отладки идёт сам. Подробности в reboot_msm.c. */
    msm_watchdog_arm();
#endif
...
#if !defined(CONFIG_DEBUG_TOUCH_PROBE) && !defined(CONFIG_NO_BOOT_ANIM)
    /* ПОКАЗ ХОДА ЗАГРУЗКИ — ТОЛЬКО ПОСЛЕ ВКЛЮЧЕНИЯ MMU И КЭША.
     *
     * Сперва он начинался сразу после подъёма журнала, то есть до MMU. С
     * выключенной трансляцией процессор считает ВСЮ память памятью
     * устройств: каждая запись уходит на шину отдельной посылкой, строго
     * по порядку, без объединения. Заливка экрана — это шесть миллионов
     * таких записей, и загрузка на них умирала, не доходя даже до
     * подъёма провода.
     *
     * Отладочная сборка этого не показывала: в ней вместо анимации
     * печатается текст, а он заливает не весь экран, а строки по мере
     * появления. Отсюда и вышло, что проверенной оказалась одна сборка,
     * а отдана другая.
     *
     * Здесь кэш уже включён, и та же заливка стоит миллисекунды. */
    boot_anim_begin();
#endif
    boot_anim_step();
    ...
    if (spmi_init()) {
        /* ПЕРВЫМ ДЕЛОМ — почему умер прошлый запуск. Железо это
    ...
    /* Канал к сопроцессору питания. Часть источников принадлежит ему, и
       иначе как просьбой их не включить.
     *
     * СТРОГО ПОСЛЕ ПРОВОДА, и не просто после, а после того, как
     * компьютер на том конце ДОГОВОРИЛСЯ с нами до конца.
     *
     * Одного порядка вызовов оказалось мало, и это стоило ещё одного
     * зависшего аппарата. Провод не поднимается сам: контроллер лишь
     * подтягивает линию, а на вопросы хоста система отвечает, крутя
     * очередь событий. Зависшая система её не крутит — и снаружи это
     * выглядит не как "зависла", а как "устройства нет". Ровно то, что
     * мы и увидели.
     *
...
void arm64_exception_handler(uint64_t index, uint64_t esr, uint64_t far, uint64_t elr) {
    ...
    /* РАССКАЗАТЬ ПО ПРОВОДУ, А ПОТОМ УЖЕ УМИРАТЬ.
     *
     * Сообщение обработчика — самое ценное, что система успевает
     * сказать, и до сих пор оно оставалось только на экране и в
     * сохраняемой области. Провод при этом был поднят и работал, но
     * останавливались мы сразу, и по нему не уходило ничего: снаружи
     * сбой выглядел неотличимо от зависания.
     *
     * Поэтому здесь система ещё полминуты крутит провод — ровно чтобы
     * отдать рассказ о собственной смерти. Заодно всё это время гладит
     * сторожевой таймер; когда перестанет, тот перезагрузит аппарат
     * сам, и круг отладки продолжится без чужой помощи. */
    for (int i = 0; i < 8000; i++) hal_time_delay_ms(1);

    /* И ВЕРНУТЬСЯ В ЗАГРУЗЧИК САМИМ.
     *
     * Остановка после сбоя означала, что круг отладки встал: аппарат
     * висел с поднятым проводом, но ни на что не отвечал, и следующий
     * заход не мог даже попросить его уйти в загрузчик. Приходилось
     * ждать сторожевого таймера или человека с кнопкой.
     *
     * Рассказав о сбое, система уходит в загрузчик — ровно туда, откуда
     * начинается следующий заход. */
#ifdef BOARD_IMEM_RESTART_REASON
