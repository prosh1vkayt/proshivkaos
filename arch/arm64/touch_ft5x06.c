...
int ft5x06_init(void) {
    ...
    /* 2. ПИТАНИЕ.
     *
     * Этого шага не хватало всё время. Обход шины вручную не нашёл по
     * адресу 0x38 никого — притом что линии стояли в единице. Единицу
     * держат внешние подтяжки, а они питаются от vcc_i2c; сам же
     * тачскрин питается ОТ ДРУГОГО источника. Одно без другого и даёт
     * ровно эту картину: шина выглядит живой, а отвечать на ней некому.
     *
     * В дереве устройств у focaltech@38 записано прямо:
     *     vdd-supply = pm8953_l10 (2.85 В)
     *     vcc_i2c-supply = pm8953_l6 (1.8 В) */
    pmic_ldo_dump(6);
    pmic_ldo_enable(6);
    pmic_ldo_dump(10);
    ...
    /* 4. СНАЧАЛА СМОТРИМ, ЖИВА ЛИ ШИНА.
     *
    ...
    /* 5. Сброс микросхемы. Делается до любого обмена и не требует шины —
    ...
    /* 7. Аппаратный контроллер. Выводы отдаём ему. */
    ...
    /* 8. Выбор пути. Аппаратный предпочтительнее — он в разы быстрее, —
 ...
 *
 * ЧИТАЕМ ДЕВЯТЬ БАЙТ, А НЕ ШЕСТЬДЕСЯТ ТРИ. Раньше вычитывался весь
 * массив на десять пальцев, хотя интерфейсу нужен ровно один указатель:
 * заголовок и первая точка. Семикратно меньше времени на шине — а шина
 * тут в главном цикле, и каждая лишняя миллисекунда на ней это
 * миллисекунда, которую интерфейс не рисует.
 *
 * ОПРАШИВАЕМ И ПО ЧАСАМ, А НЕ ТОЛЬКО ПО ЛИНИИ ПРЕРЫВАНИЯ. Вот это и
 * было причиной залипания клавиш. Контроллер сообщает об отпускании
 * пальца ОДИН раз: дёргает линию, отдаёт пакет с нулём точек и
 * успокаивается. Если в этот момент система занята — а она была занята
 * отрисовкой кадра по полсекунды, — сообщение пропадает. Линия снова в
 * единице, новых данных нет, и для системы палец остаётся прижатым
 * навсегда. Экранная клавиатура при этом честно держит клавишу
 * подсвеченной и не выдаёт символ: отпускания-то не было.
 *
 * Поэтому пока палец прижат, контроллер опрашивается по часам, и
 * отпускание не может потеряться в принципе. В покое опрос редкий:
...
int ft5x06_poll(int *x, int *y, int *pressed) {
    ...
    int moved = (nx > g_x ? nx - g_x : g_x - nx) >= TS_MOVE_EPSILON ||
                (ny > g_y ? ny - g_y : g_y - ny) >= TS_MOVE_EPSILON;

    int changed = (now_pressed != g_pressed) || (now_pressed && moved);

    /* Запомненную точку обновляем ТОЛЬКО когда о ней доложили.
     *
     * Иначе порог считается от предыдущего опроса, а не от последнего
     * сообщённого положения, и палец уползает сколь угодно далеко
     * шагами по одному пикселю — ни одного события движения при этом не
     * возникает, а нажатие срабатывает уже на другой клавише. */
    if (changed) { g_x = nx; g_y = ny; }
