...
void early_con_color(const char *s, uint8_t r, uint8_t g, uint8_t b) {
    /* Всё, что появляется на экране, дублируется в постоянный журнал.
     *
     * Экран читается только глазами и только пока горит; журнал переживает
     * перезагрузку и вытаскивается на компьютер (см. tools/posdev). Пока
     * эти два вывода расходились, самое ценное — подробности отказа
