...
/* На PC-совместимой машине никакого описания платы нет и не нужно:
 * адреса периферии стандартизованы с 1981 года. */
const char *hal_board_name(void) { return "PC"; }
...
void hal_parallel(void (*fn)(void *arg, int part, int parts), void *arg) { fn(arg, 0, 1); }
...
void hal_arch_init(void *boot_info) {
    /* На x86 к моменту входа в kmain процессор уже в защищённом режиме с
       ...
       Multiboot пока незачем: карта памяти не используется, адреса
       периферии на PC-совместимой машине и так стандартные.
       Парная реализация для ARM64 (arch/arm64/cpu.c) делает основную
       работу: разбирает device tree и включает MMU. */
    (void)boot_info;
}
