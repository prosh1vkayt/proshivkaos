...
void hal_input_init(void) {
    ...
    if (g_device_count == 0) {
        ...
        /* Дышащий красный ТОЛЬКО ПРИ ОТКАЗЕ.
         *
         * Раньше он загорался в любом случае, и это было правильно, пока
         * успех оставался умозрительным: система всё равно ничего не
         * умела показать. Теперь успех означает работающий тачскрин, и
         * останавливать систему на нём — значит не дать его потрогать.
         * Журнал при этом не теряется: он пишется в сохраняемую область
         * непрерывно, а не в конце. */
        if (!g_touch_ok) early_fb_alert();
#endif
