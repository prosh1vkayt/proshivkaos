...
fonts:
	...
	python3 tools/icongen.py > gui/icon_data.c
...
ifeq ($(ARCH),x86)
  ...
  ifneq ($(call cfg,FIRMWARE),)
    ARCH_BASE_SOURCES += arch/arm64/firmware.c arch/arm64/pil.c arch/arm64/smd.c arch/arm64/wcnss_ctrl.c arch/arm64/wlan.c
  endif
...
#
# То же самое с флагами из командной строки (EXTRA_CFLAGS, их передаёт
# posdev: отладочные признаки, опыты). Файл с флагами переписывается,
# только когда они изменились, и объекты от него зависят. Без этого
# «сборка с признаком» молча отдавала прежний образ — так потерян ещё один
# заход: многоядерность «не заработала», потому что её вызова в образе
# просто не было.
CFLAGS_STAMP := $(OBJDIR)/.cflags
$(shell mkdir -p $(OBJDIR); printf '%s\n' '$(subst ','\'',$(CFLAGS))' | cmp -s - $(CFLAGS_STAMP) || printf '%s\n' '$(subst ','\'',$(CFLAGS))' > $(CFLAGS_STAMP))

$(OBJDIR)/%.o: %.c $(BOARD_CONFIG) $(CFLAGS_STAMP)
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS) -MMD -MP -c $< -o $@

$(OBJDIR)/%.o: %.S $(CFLAGS_STAMP)
	@mkdir -p $(dir $@)
	$(CC) $(CFLAGS) -MMD -MP -c $< -o $@
